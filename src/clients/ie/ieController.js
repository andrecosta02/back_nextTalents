...
module.exports = {
  ...
  login: async (req, res) => {
    ...
    try {
      const ie = await ieService.getUserByEmail(email);
      ...
      const passwordMatch = await bcrypt.compare(pass, ie.pass);
      if (!passwordMatch) {
        return res.status(401).json({
          statusCode: 401,
          message: "E-mail ou senha inválidos.",
  ...
  forgotPass: async (req, res) => {
    ...
    const token = jwt.sign({ id: ie.id, type: 'ie' }, SECRET, { expiresIn: '1h' });
    const expiresAt = new Date(Date.now() + 60 * 60 * 1000);
    await ieService.saveResetToken(ie.id, token, expiresAt);
    const emailTitle = "Recuperação de senha Next Talents";
    const htmlEnv = await htmlEmail.resetPass(ie.nome, token);
    await clientEmail.envEmail(email, emailTitle, '', htmlEnv);
    return res.status(200).json({ message: "If this email exists, a reset link has been sent." });
  },
