...
module.exports = {
  /**
   * Busca uma IE pelo e-mail
   */
  getUserByEmail: (email) => {
    return new Promise((resolve, reject) => {
      const query = "SELECT * FROM ie WHERE email = ?";
      const values = [email];
  ...
  /**
   * Registra uma nova IE
   */
  register: (nome, unit, email, pass, cnpj) => {
    return new Promise((resolve, reject) => {
      // Verifica existência prévia
      let query = "SELECT * FROM ie WHERE email = ? OR cnpj = ?";
      ...
      db.query(query, values, (err, results) => {
        ...
        if (results && results.length > 0) {
          // Já existe IE com mesmo e-mail ou cnpj
          return resolve({
            code: "2",
        ...
        // Insere nova IE
        query = "INSERT INTO ie (nome, unit, email, pass, cnpj) VALUES (?, ?, ?, ?, ?)";
  ...
  /**
   * Atualiza campos permitidos de uma IE
   */
  updateIeById: (ieId, updates) => {
    return new Promise((resolve, reject) => {
      const fields = Object.keys(updates);
      const setClause = fields.map(f => `${f} = ?`).join(", ");
      const values = fields.map(f => updates[f]).concat(ieId);
      const query = `UPDATE ie SET ${setClause} WHERE id = ?`;
  ...
  findResetToken: (token) => {
    return new Promise((resolve, reject) => {
      const query = `
        ...
        WHERE token = ? AND user_type = 'ie' AND used = FALSE AND expires_at > NOW()`;
      const values = [token];
  ...
  /**
   * Atualiza a senha da IE
   */
  updatePassword: (userId, newPasswordHash) => {
    return new Promise((resolve, reject) => {
      const query = "UPDATE ie SET pass = ? WHERE id = ?";
  ...
  /**
   * Ativa a IE após confirmação de e-mail
   */
  activateIeById: (id) => {
    return new Promise((resolve, reject) => {
      const query = "UPDATE ie SET is_active = TRUE WHERE id = ?";
      const values = [id];
